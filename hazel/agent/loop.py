...
from hazel.agent.skills import BUILTIN_SKILLS_DIR
...
class AgentLoop:
    ...
    def _register_default_tools(self) -> None:
        ...
        for cls in (
            ...
            IntentCompleteTool, IntentSnoozeTool, IntentDeferTool,
            IntentListDueTool, IntentSyncLinksTool,
        ):
    ...
    async def _connect_mcp(self) -> None:
        ...
        from hazel.agent.tools.mcp import connect_mcp_servers
        try:
    ...
    async def _handle_restart(self, msg: InboundMessage) -> None:
        ...
        async def _do_restart():
            ...
            # Use -m hazel instead of sys.argv[0] for Windows compatibility
            # (sys.argv[0] may be just "hazel" without full path on Windows)
            os.execv(sys.executable, [sys.executable, "-m", "hazel"] + sys.argv[1:])
    ...
    async def _process_message(
        ...
        if cmd == "/help":
            lines = [
                ...
                "/compact — Manually trigger memory consolidation",
                "/help — Show available commands",
            ]
            return OutboundMessage(
                channel=msg.channel, chat_id=msg.chat_id, content="\n".join(lines),
            )
        if cmd in ("/context", "/tokens"):
            estimated, source = self.memory_consolidator.estimate_session_prompt_tokens(session)
            window = self.context_window_tokens
            pct = (estimated / window * 100) if window > 0 else 0
            trigger = int(window * self.memory_consolidator._CONSOLIDATION_TRIGGER_RATIO)
            lines = [
                f"Context: {estimated:,} / {window:,} tokens ({pct:.0f}%)",
                f"Auto-compact trigger: {trigger:,} tokens ({int(self.memory_consolidator._CONSOLIDATION_TRIGGER_RATIO * 100)}%)",
                f"Compact target: {window // 2:,} tokens (50%)",
                f"Messages: {len(session.messages)} ({session.last_consolidated} consolidated)",
                f"Estimation: {source}",
            ]
            return OutboundMessage(
                channel=msg.channel, chat_id=msg.chat_id, content="\n".join(lines),
        ...
        if cmd == "/compact":
            estimated_before, _ = self.memory_consolidator.estimate_session_prompt_tokens(session)
            await self.memory_consolidator.maybe_consolidate_by_tokens(session, force=True)
            estimated_after, _ = self.memory_consolidator.estimate_session_prompt_tokens(session)
            delta = estimated_before - estimated_after
            if delta > 0:
                content = (
                    f"Compacted: {estimated_before:,} → {estimated_after:,} tokens "
                    f"(freed {delta:,})"
                )
            else:
                content = f"Nothing to compact ({estimated_before:,} tokens)."
            return OutboundMessage(
                channel=msg.channel, chat_id=msg.chat_id, content=content,
            )
        await self.memory_consolidator.maybe_consolidate_by_tokens(session)
    ...
    def _save_turn(self, session: Session, messages: list[dict], skip: int) -> None:
        ...
        self._prune_old_tool_results(session)

    _PRUNING_MIN_CHARS = 500
    _PRUNING_KEEP_TURNS = 2

    def _prune_old_tool_results(self, session: Session) -> None:
        """Replace old large tool results with short summaries to save context."""
        messages = session.messages
        if not messages:
            return

        # Find turn boundaries by locating user messages
        user_indices = [i for i, m in enumerate(messages) if m.get("role") == "user"]
        if len(user_indices) <= self._PRUNING_KEEP_TURNS:
            return

        # Everything before the Nth-from-last user message is prunable
        prune_before = user_indices[-self._PRUNING_KEEP_TURNS]

        # Build map of tool_call_id -> key_arg from assistant messages in the prunable range
        tc_args: dict[str, str] = {}
        for m in messages[:prune_before]:
            if m.get("role") == "assistant" and m.get("tool_calls"):
                for tc in m["tool_calls"]:
                    ...
                    try:
                        ...
                        args = {}
                    if tc_id:
                        tc_args[tc_id] = self._extract_key_arg(name, args)
        ...
        for i in range(prune_before):
            ...
            if not isinstance(content, str) or len(content) <= self._PRUNING_MIN_CHARS:
                continue
            tool_name = m.get("name", "tool")
            key_arg = tc_args.get(m.get("tool_call_id", ""), "")
            char_count = len(content)
            if key_arg:
                m["content"] = f"[{tool_name}: {key_arg}, {char_count:,} chars]"
            else:
                m["content"] = f"[{tool_name}: {char_count:,} chars]"

    @staticmethod
    def _extract_key_arg(tool_name: str, args: dict) -> str:
        """Extract the most meaningful argument for a tool result summary."""
